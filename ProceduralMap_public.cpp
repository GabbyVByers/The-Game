...
void ProceduralMap::GenerateWorldMap(sf::Image& worldMap, int setWidth, unsigned int seed) {
	State& s = GetState();
	...
	worldMap = worldLayerProgression[worldLayerProgression.size() - 1];
}

void ProceduralMap::DisplayWorldMap(sf::RenderWindow& window) {
	State& s = GetState();
	std::vector<sf::Image>& worldLayerProgression = s.worldLayerProgression;
	unsigned int& width = s.width;
